...
user_states = {}          # User ၏ လက်ရှိအခြေအနေ (ဥပမာ - URL တောင်းနေစဉ်)
user_cooldowns = {}       
known_users = set()       
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_states.get(user_id) == "waiting_for_url":
        if "portal-as.ruijienetworks.com" in text or "mac=" in text:
            ...
            user_states[user_id] = None  # State ရှင်းထုတ်မည်
            ...
            await update.message.reply_text(
                ...
                "အောက်ပါခလုတ်ကိုနှိပ်၍ Free ကုဒ်စတင်ရှာဖွေနိုင်ပါပြီ။",
                reply_markup=InlineKeyboardMarkup(keyboard),
                parse_mode='Markdown'
            )
        else:
...
# ── USER 1-HOUR BACKGROUND SEARCH TASK ───────────────────────────────────

async def run_user_1hour_search(user_id, chat_id, context, portal_url):
    code_gen = iter_digit_codes(6)
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "check_sub":
        ...
        await query.edit_message_text(
            "👋 **ကြိုဆိုပါတယ် Bot User များရှင်** 👋\n\n"
            f"🔗 **သင့်ရဲ့ URL:** `{user_url}`\n\n"
            "ချန်နယ် Join ခြင်း အောင်မြင်ပါသည်။ အောက်ပါခလုတ်မှတဆင့် Portal URL ထည့်သွင်းပြီး Free ကုဒ်များကို ရှာဖွေနိုင်ပါပြီ။",
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode='Markdown'
        )
        return
    ...
    if data == "start_user_search":
        ...
        user_search_tasks[user_id] = {
            'running': True,
            'task': asyncio.create_task(run_user_1hour_search(user_id, query.message.chat_id, context, user_urls[user_id]))
        }
        ...
        await query.edit_message_text(
            "🚀 **Free ကုဒ် 1 နာရီ ရှာဖွေမှု စတင်နေပါပြီ...**\n\n"
            "• ကုဒ်တွေ့ရှိပါက သို့မဟုတ် **၁ နာရီ** ပြည့်သွားပါက ရှာဖွေမှု အလိုအလျောက် ရပ်သွားပါမည်။\n"
            "• တွေ့ရှိလာပါက Bot မှ ချက်ချင်း မက်ဆေ့ချ်ပို့ပေးပါမည်။ ခဏစောင့်ပေးပါ။",
    ...
    if data == "user_menu":
        ...
        await query.edit_message_text(
            "👋 **ကြိုဆိုပါတယ် Bot User များရှင်** 👋\n\n"
            f"🔗 **သင့်ရဲ့ URL:** `{user_url}`\n\n"
            "ဤ Botမှတဆင့် Free ကုဒ်များကို ကိုယ်တိုင်ရှာဖွေနိုင်ပါသည်။",
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode='Markdown'
        )
        return

    # Admin Authorization Check for other callbacks
    if not is_admin(user_id):
...
def main():
    ...
    print("🤖 Telegram Bot Starting (User Custom URL & Force Subscribe Active)...")
    app.run_polling()
